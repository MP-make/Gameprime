...
// === ACTUALIZAR PRODUCTO ===
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'update_producto') {
    $id = intval($_POST['id'] ?? 0);
    $titulo = $_POST['titulo'] ?? '';
    $categoria = $_POST['categoria'] ?? '';
    $precio = floatval($_POST['precio'] ?? 0);
    ...
    try {
        $stmt = $conn->prepare("
            UPDATE productos
            SET titulo=:titulo, categoria=:categoria, precio=:precio, fecha_lanzamiento=:fecha, descuento=:descuento
            WHERE id=:id
        ");
        $stmt->execute([
            ':titulo' => $titulo,
